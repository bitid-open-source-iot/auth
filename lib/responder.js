		...
		response: {
			...
			apps: {
				...
				get: (result) => {
					...
					deferred.resolve({
						'url': result.url,
						'urlPrivacyPolicy': result.urlPrivacyPolicy,
						'urlTermsAndConditions': result.urlTermsAndConditions,
						'role': result.role,
						'icon': result.icon,
						'name': result.name,
			...
			auth: {
				...
				validate: (result) => {
					...
					deferred.resolve({
						'appId': result.apps,
						'userId': result.req.body.header.userId,
		...
		model: (req, result) => {
			...
			switch (req.originalUrl) {
				...
				case ('/auth/delete-account'):
					responder.response.auth.deleteAccount(result).then(deferred.resolve, deferred.reject);
					break;
		...
		error: (req, res, err) => {
			...
			try {
				if (typeof (err) != 'undefined') {
					...
					send(payload.code, payload);
				} else {
					var logged = tools.log('error', 'Error in responder.error', 'Internal server error!', { reqOriginalUrl: req?.originalUrl, reqBody: req?.body, reqAuthorization: req?.authorization });
					logged.error.errors[0].code = 503;
					logged.error.errors[0].reason = 'Internal server error!';
					logged.error.errors[0].message = 'Internal server error!';
					send(logged.error.code, logged.error);
				};
			} catch (e) {
				send(503, {
					'code': 503,
		...
		success: (req, res, result) => {
			responder.model(req, result)
				.then(result => {
					...
					var err = tools.log('error', 'Error in responder.success', error, { reqOriginalUrl: req.originalUrl, reqBody: req?.body, reqAuthorization: req?.authorization });
