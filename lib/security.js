...
async function authorizeEmailChange({ user, password, newEmail }) {
    const email = normalizeEmail(newEmail);
    if (!email) {
        return { allowed: false, code: 400, message: 'Invalid email format' };
    }

    if (!user) {
        return { allowed: false, code: 401, message: 'Invalid credentials' };
    }
